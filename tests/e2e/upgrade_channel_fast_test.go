...
// - It sends check requests with "channel": "fast" in the body
// - The channel_aware stub responds with update_available (fast → 0.17.0→0.18.0)
// - An upgrade Job is created and succeeds
// - The Helm release is upgraded to TargetChartVersion (0.18.0)
// - A success report is sent
func TestChannelFastOnly(t *testing.T) {
	ctx := context.Background()

	// Step 1: Configure stub to respond in channel_aware mode.
	...
	// Step 2: Install the initial chart version with auto-upgrade enabled and channel=fast.
	values := map[string]string{
		...
		// Channel selection — maps to KUBEADAPT_UPGRADE_CHANNEL=fast in the pod
		"agent.autoUpgrade.channel": "fast",
	}
	...
	// Step 6: Assert the stub received at least one check request with channel=fast.
	...
	foundFastChannel := false
	for _, raw := range checks {
		var body checkReqBody
		if err := json.Unmarshal(raw, &body); err == nil && body.Channel == "fast" {
			foundFastChannel = true
			break
		}
	}
	if !foundFastChannel {
		t.Errorf("expected at least one check request with channel=fast, got: %v", checks)
	}
